...
export default function Input({ label, placeholder, className, type }: InputProps) {

    const labelStyles = 'text-[14px] text-[#003C71] font-sst font-[700]';

    const inputBaseStyles = clsx(
        'w-full focus:outline-none placeholder:text-[16px] font-sst placeholder:font-[500]',
        MAPPING_INPUT_BASE_STYLE_DEFAULT[type || 'main'],
    );

    const inputStyles = clsx(
        'w-full h-full p-[10px] rounded-[15px]',
        MAPPING_INPUT_WRAPPER_STYLE_DEFAULT[type || 'main'],
        className,
    )
    ...
    return (
        <div className={inputStyles}>
            ...
            <input className={inputBaseStyles} id={label} type="text" placeholder={placeholder} />
        </div>
    )
}
